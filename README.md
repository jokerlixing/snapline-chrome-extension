...
- **宽度与大小**：自定义宽度为 200–7680 的整数像素；输出单边最多 32,760 像素、总像素最多 6,400 万，1–3 倍清晰度均计入该限制。请求的图片超限时，网页版和插件版会自动等比缩小并提示实际尺寸，保留整页内容；自动缩小的目标最多为单边 30,000 像素、总像素 4,800 万。输入宽度决定排版宽度，固定宽布局可能溢出，主滚动区域截图也可能比输入值窄。
...
1. 从 [Releases](https://github.com/jokerlixing/snapline-chrome-extension/releases/latest) 下载 `Snapline-v1.2.4.zip` 并完整解压。
...
- 构建同时生成 `dist/Snapline-v1.2.4.zip` 安装包，内容与 `dist/snapline/` 相同（顶层为 `snapline` 文件夹），也是发布到 Releases 的那个文件。
...
**最近核查：2026-09-23，v1.2.4。** 36 项单元测试、17 组长网页与重载检查、17 项插件工作台、8 组控件、8 组内部滚动、5 组长页分段、25 个格式检查、5 项升级、19 组网页版捕获、11 组网页操作、12 组文档化选项全部通过（浏览器检查共 127 项，0 失败）。

近期重点修复：

- **超大图片转换**：`2,352 × 45,498` 像素的整页曾直接报尺寸超限。现在两端自动等比缩小为 `1,550 × 30,000` 像素，保留首尾与滚动内容，并明确提示缩小后的尺寸；真实浏览器中已完成 PNG、JPG、WebP、PDF 下载验证。
...
功能核查记录及每项对应的测试命令见 [最新功能验收](docs/verification.md#2026-09-23-超大图片完整转换修复)。线上复测可将环境变量 `SNAPLINE_WEB_URL` 设为公共网站地址后运行 `npm run test:web`。测试中使用可复现的本地页面和独立浏览器配置，不代表任意第三方网站或私人登录会话均已实测。
