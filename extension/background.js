...
import { normalizeCaptureOptions, normalizeWebUrl, isCapturableUrl, calculateCaptureGeometry, captureResizeWarning, utf8ToBase64, base64ToBlob, captureErrorMessage, planSurfaceTiles, SURFACE_LIMIT, MIN_TILE_DIMENSION } from './lib/capture-utils.js';
...
// instead of failing.
async function captureSurface(job, clip, scale, contextId, captureBeyondViewport = true, onProgress, outputScale = scale, roundUp = false) {
  ...
  for (let attempt = 0; attempt < SURFACE_ATTEMPTS; attempt++) {
    ...
    let canvas;
    try {
      if (plan.tiles.length === 1 && outputScale === scale) {
      ...
      const outputDimension = value => Math.max(1, (roundUp ? Math.ceil : Math.floor)(value * outputScale));
      canvas = new OffscreenCanvas(outputDimension(clip.width), outputDimension(clip.height));
      ...
      for (let index = 0; index < plan.tiles.length; index++) {
        ...
        const shot = await command(job, 'Page.captureScreenshot', screenshotRequest(tile, captureBeyondViewport || plan.tiles.length > 1), 60000);
        if (!shot.data) throw new Error('没有收到截图数据，请重新生成。');
        ...
        try {
          ...
          if (outputScale === scale) drawing.drawImage(bitmap, Math.round((tile.x - plan.origin.x) * scale), Math.round((tile.y - plan.origin.y) * scale));
          else {
            const left = Math.floor((tile.x - plan.origin.x) * outputScale);
            const top = Math.floor((tile.y - plan.origin.y) * outputScale);
            const right = Math.floor((tile.x + tile.width - plan.origin.x) * outputScale);
            const bottom = Math.floor((tile.y + tile.height - plan.origin.y) * outputScale);
            if (right > left && bottom > top) drawing.drawImage(bitmap, 0, 0, bitmap.width, bitmap.height, left, top, right - left, bottom - top);
          }
...
async function captureSurfaceBlob(job, clip, scale, contextId, captureBeyondViewport, onProgress, outputScale = scale) {
  const surface = await captureSurface(job, clip, scale, contextId, captureBeyondViewport, onProgress, outputScale);
...
async function captureScrollRegion(job, contextId, options, warnings) {
  ...
  try {
    ...
    for (let step = 0; covered < geometry.height; step++) {
      ...
      region = await evaluate(job, scrollRegion, ['scroll', { top: covered / geometry.outputScale, assets: options.lazyLoad, wait: Math.max(180, Math.min(options.delay, 1000)) }], contextId);
      ...
      const tile = await captureSurface(job, { x: region.x, y: region.y, width: region.width, height: region.height }, options.scale, contextId, false, undefined, geometry.outputScale, true);
      ...
      try {
        ...
        const offset = covered - Math.floor(region.top * geometry.outputScale);
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        if (nested) { geometry = nested.geometry; blob = nested.blob; }
          ...
          const resizeWarning = captureResizeWarning(geometry, options);
          if (resizeWarning) attemptWarnings.push(resizeWarning);
          ...
          blob = await captureSurfaceBlob(job, geometry.clip, options.scale, contextId, options.scope === 'full',
            percent => progress('正在分段拼接完整截图', 72 + Math.round(percent * 0.14)), geometry.outputScale);
        }
        checkJob(job);
